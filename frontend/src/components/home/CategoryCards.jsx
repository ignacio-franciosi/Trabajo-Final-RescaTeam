...
const HeartIcon = ({ className = 'w-16 h-16 text-rose-600' }) => (
...
const SearchIcon = ({ className = 'w-16 h-16 text-amber-600' }) => (
...
const HomeSolidIcon = ({ className = 'w-16 h-16 text-emerald-600' }) => (
...
const Card = ({ title, description, actionLabel, to, colorClass, icon, count, countSuffix }) => {
    const navigate = useNavigate();
    return (
        <div className={`rounded-xl shadow-md p-6 bg-white border ${colorClass} text-center flex flex-col items-center`}>
            <div className="mb-3">
                {icon}
            </div>
