        ...
        <div className="grid gap-6 md:grid-cols-3">
          {cards.map(card => (
            <button
              ...
              className={`relative group text-left bg-white rounded-xl shadow hover:shadow-lg p-6 transition border-t-4 ${card.borderColor} ${card.hoverBorderColor}`}
            >
              <h2 className={`text-xl font-semibold mb-2 ${card.hoverTextColor}`}>{card.title}</h2>
              <p className="text-sm text-gray-600 leading-snug">{card.description}</p>
              <span className={`inline-block mt-4 ${card.textColor} font-medium group-hover:underline`}>Continuar →</span>
            </button>
          ))}
        </div>
      </div>
