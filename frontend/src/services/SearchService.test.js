...
describe('SearchService', () => {
  ...
  describe('getSimilarPets', () => {
    it('should get similar pets successfully', async () => {
      ...
      const result = await getSimilarPets('post123', 'adoption')

      expect(result.success).toBe(true)
      expect(result.data).toEqual(mockResponse.data)
      expect(apiSearch.get).toHaveBeenCalledWith('/vectors/search?post_id=post123&post_type=adoption')
    })
    ...
    it('should handle different post types', async () => {
      ...
      const result = await getSimilarPets('post456', 'lost')

      expect(result.success).toBe(true)
      expect(apiSearch.get).toHaveBeenCalledWith('/vectors/search?post_id=post456&post_type=lost')
    })
