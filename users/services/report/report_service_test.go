...
func TestUpdateReport_Success(t *testing.T) {
	...
	adminComment := "Reporte resuelto"
	updateDto := dto.ReportDto{
		ReportId:     1,
		AdminComment: &adminComment,
		ReportStatus: "resolved",
	}

	updatedReport := model.Report{
		ReportId:          1,
		UserId:            5,
		PostId:            "post123",
		ComplainingUserId: 2,
	...
	mockClient.On("UpdateReport", mock.MatchedBy(func(r model.Report) bool {
		return r.ReportId == 1 && r.ReportStatus == "resolved" && r.AdminComment != nil && *r.AdminComment == adminComment
	})).Return(updatedReport, nil)

	result, err := services.ReportService.UpdateReport(updateDto)

	assert.Nil(t, err)
	assert.Equal(t, 1, result.ReportId)
	assert.Equal(t, "resolved", result.ReportStatus)
	assert.NotNil(t, result.AdminComment)
	assert.Equal(t, adminComment, *result.AdminComment)
	mockClient.AssertExpectations(t)
}

func TestUpdateReport_Error(t *testing.T) {
	mockClient := new(mockReportClient)
	clients.ReportClient = mockClient
	...
	mockClient.On("UpdateReport", mock.Anything).Return(model.Report{}, errors.New("database error"))
...
func TestDeleteReport_Success(t *testing.T) {
	...
	mockClient.On("GetReportById", 1).Return(report)
	mockClient.On("DeleteReport", report).Return(nil)

	err := services.ReportService.DeleteReport(1)

	assert.Nil(t, err)
	mockClient.AssertExpectations(t)
}

func TestDeleteReport_Error_NotFound(t *testing.T) {
	mockClient := new(mockReportClient)
	clients.ReportClient = mockClient

	mockClient.On("GetReportById", 999).Return(model.Report{})

	err := services.ReportService.DeleteReport(999)
...
func TestDeleteReport_Error_DeleteFailed(t *testing.T) {
	mockClient := new(mockReportClient)
	clients.ReportClient = mockClient
	...
	mockClient.On("GetReportById", 1).Return(report)
	mockClient.On("DeleteReport", report).Return(errors.New("database error"))
